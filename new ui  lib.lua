...
function Library.MakeWindow(Size, Subtext, ColorTheme)
	...
	NavBar.Position = UDim2.new(0, 11, 0, 66)
	...
	local function Prompt(Details)
		...
		WindowConnections:Add(Button1.InputBegan:Connect(function(Key)
		...
		if Details.Buttons ~= 1 then
			WindowConnections:Add(Button2.InputBegan:Connect(function(Key)
				if WindowConnections:IsPress(Key) then
	...
	WindowConnections:Add(Exit.InputEnded:Connect(function(Key)
		if WindowConnections:IsPress(Key) then
			Prompt({
	...
	;(function()
		...
		WindowConnections:Add(Resize.InputBegan:Connect(function(Key)
			if WindowConnections:IsPress(Key) then
				IsResizing = true
	...
	function WindowData.MakeTab(Name, Icon, PreviewText)
		...
		WindowConnections:Add(TabFrame.InputEnded:Connect(function(Key)
			if WindowConnections:IsPress(Key) then
				for i, v in pairs(TabsData.Buttons) do
					if v == TabFrame then continue end
					local v = v[1]
		...
		function PageData.Button(Name, Callback)
			...
			WindowConnections:Add(Button.InputBegan:Connect(function(Key)
			...
			WindowConnections:Add(Button.InputEnded:Connect(function(Key)
				if WindowConnections:IsPress(Key) and not IsActivePrompt() then
		...
		function PageData.Dropdown(Name, Array, DefaultValue, Callback)
			...
			function DropdownData:Refresh(NewArray)
				...
				for i, v in pairs(NewArray) do
					...
					WindowConnections:Add(TextButton.InputEnded:Connect(function(Key)
						if WindowConnections:IsPress(Key) then
							IsActivated = false
			...
			WindowConnections:Add(Dropdown.InputEnded:Connect(function(Key)
				if WindowConnections:IsPress(Key) then
		...
		function PageData.Slider(Name, MinValue, MaxValue, DefaultValue, Callback)
			...
			local Last
			WindowConnections:Add(Bar.InputBegan:Connect(function(Key)
				if WindowConnections:IsPress(Key) then
					Dragging = true
					while Dragging do
						if IsActivePrompt() then
		...
		function PageData.Toggle(Name, Callback, Enabled)
			...
			WindowConnections:Add(Toggle.InputEnded:Connect(function(Key)
				if WindowConnections:IsPress(Key) then
					Enabled = not Enabled
...
local function Test()
	local Window = Library.MakeWindow(nil, "Block fruts", Color3.fromRGB(47, 255, 255))
	local Tab = Window.MakeTab("Auto Fuck", "rbxassetid://125135677484819", "fucking yess dude")
	local Tab2 = Window.MakeTab("Business", "rbxassetid://125135677484819")
	local RedValue
	local GreenValue
	local BlueValue
	Tab.Section("Main skibidi")
	Tab.Toggle("Penis fuck", print, false)
	Tab.Button("Test Prompt", function()
		Window:Prompt({
			Title = "This is a test!",
			Text = "it is a test, will you press yes or no? i dont know, if you press yes it might print something though! i will put alot of text here to test how good this prompt system is lorem ipsum however it goes 1234567890 abcdefghij klmnop qrstuv wxyz!?",
			Button1Text = "Wheres the yes button",
			Buttons = 1,
			Callback = function(l)
				Window:UpdateColorTheme(Color3.fromRGB(RedValue:GetValue(), GreenValue:GetValue(), BlueValue:GetValue()))
			end,
		})
	end)
	Tab.Section("Sliders")
	RedValue = Tab.Slider("Red", 0, 255, 47)
	GreenValue = Tab.Slider("Green", 0, 255, 255)
	BlueValue = Tab.Slider("Blue", 0, 255, 255)
	Tab.Section("Dropdown")
	Tab.Dropdown("Ohio", {"Bananas", "Apples", "Kiwis", "Pears", "Oranges", "Avocados"}, "Kiwi", function(l)
		print(l)
	end)
	Tab.Section("Input Boxes")
	Tab.TextBox("Target Name", "", "Username", function(text)
		print(text)
	end)
end
if IsStudio or game.PlaceId == 71236979615716 then
	Players.LocalPlayer.PlayerGui:WaitForChild("debug_"):Destroy()
	Test()
end
